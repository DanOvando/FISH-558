#================================================================
#Lab 5 for FISH458: PART 3 predator self-limitation
...
#shape of the predator self-limitation plot
self.limitation.plot <-  function(a=0.05,c=2000) {
   n <- 100
   deaths.vec <- seq(0,2000000,length.out=n)
   recruits <- vector(length=n)
   for (i in 1:n) {
      recruits[i] <- a*deaths.vec[i]/(1+a/c*deaths.vec[i])
   }
   plot(x=deaths.vec, y=recruits, type="l", xaxs="i", yaxs="i")
}
self.limitation.plot()


#predator prey with limitation on births
predator.prey <- function(wild0, wildK, lion0, killrate, assimil, nyears, 
                          wildr=0.2, lionsurv=0.8,  
                          arrow.length=0.0, maxbirths=2000) {
   ...
   for (i in 1:(nyears-1)) {
      deaths <- N.wild[i]*(1-exp(-1*killrate*N.lions[i]))
      N.wild[i+1] <- N.wild[i]+wildr*N.wild[i]*(1-N.wild[i]/wildK)-deaths
      N.lions[i+1] <- lionsurv*N.lions[i]+ assimil*deaths/(1+assimil*deaths/maxbirths)
   }
...
predator.prey(wild0=1000000, lion0=8000, wildr=0.2, wildK=1500000, lionsurv=0.8, 
              killrate=0.000016, assimil=0.05, nyears=300, arrow.length=0.0, maxbirths=2000)
