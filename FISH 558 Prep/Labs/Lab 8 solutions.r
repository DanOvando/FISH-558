...
DispersalKernel <- function(sigma, ncells) {
   ...
   ## moving from i to j.
   dk <- matrix(nrow=ncells,ncol=ncells)
   for (i in 1:ncells)  {               # loop over cells:  from
      for (j in 1:ncells)  {           #loop over cells: to
         ...
         wrapdistance1 <- abs(i+ncells-j)
         ## wrap around distance when j is small and i is high
         wrapdistance2 <- abs(i-ncells-j)
...
#explore different values of sigma and ncells
round(DispersalKernel(sigma=0.5, ncells=5),4)
round(DispersalKernel(sigma=0.5, ncells=3),4)
colSums(DispersalKernel(sigma=3, ncells=5))
...
#the movement matrix. Use sigma = 0.5
ncells <- 7
M <- DispersalKernel(sigma=0.5, ncells=ncells)

#**create abundance matrix N with nyear number of rows 
#and ncells number of columns 
nyear <- 100
...
plot(0,xlim=c(0,nyear+1), ylim=c(0,max(N[1,])), type="n", xlab="Time", ylab="Abundance")
for(i in 1:ncol(N)) {
   lines(x=1:nyear, y=N[,i], col=rainbow(ncells)[i],lwd=2)
}
...
MPA.model.lab8 <- function(r, K, u.out, ncells, nyear, MPA.width, sigma) {
   ...
   plot(x=1:ncells, y=N[1,], xlab="Cell number", lwd=3,
        ylab="Population size", ylim=c(0, 1.05*max(N[1,])), type="l", yaxs="i", xaxs="i")
