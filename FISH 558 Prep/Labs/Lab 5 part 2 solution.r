...
predator.prey.iso <- function(wild0, wildK, lion0, killrate, assimil, nyears, 
   ...
   #iso.prey (vector of isocline for prey)
   #iso.predator (vector of isocline for predators)
   iso.lions <- seq(0,1.05*max(N.lions),length.out=100)
   iso.prey <- vector(length=length(iso.lions))
   iso.predator <- vector(length=length(iso.lions))
   
   #now calculate the isoclines (the number of wildebeest for every iso.lion value)
   for (i in 1:length(iso.lions)) {
      iso.prey[i] <- wildK/wildr*(wildr-(1-exp(-1*killrate*iso.lions[i])))   
      iso.predator[i] <- iso.lions[i]/assimil*(1-lionsurv)/(1-exp(-1*killrate*iso.lions[i]))
   }
