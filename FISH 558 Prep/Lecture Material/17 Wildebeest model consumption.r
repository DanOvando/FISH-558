...
##The animation output requires installation of ImageMagick, 
##www.imagemagick.org/script/index.php, followed by machine restart
##This model includes grass consumption.
##maxcons = maximum food consumed per wildebeest
##halfcons = food level for half consumption
################################################################################
run.model <- function(Xbar=8.1, Ybar=8.1, Xsigma=0.001,Ysigma=0.001,nX=10,nY=10,dBar=0.1,
                      grassEven=T,nWild=500,nYear=50, movie.file="wildebeest40consume.gif", 
                      MaxCons=0.05, HalfCons=10,
  ...
  for (yr in 1:nYear) {
    ...
    #calculate  grazing and update the grass amount
    for (i in 1:nX){
      for (j in 1:nY) {
         consumed <- MaxCons * Grass[i,j] / (HalfCons + Grass[i, j]) #consumed per wildebeest
         pConsumed <- 1-exp(-1*consumed*Wild[i,j])
         Grass[i,j] <- Grass[i,j]*(1-pConsumed)
      }
    }
...
run.model(grassEven=T, movie.file="wildebeest50consum.gif")
